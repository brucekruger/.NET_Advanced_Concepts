...
public class CartV2ControllerTests
{
    private readonly CartController _cartV2Controller;
    private readonly Mock<ICartService> _mockCartService = new();
    private readonly Fixture _fixture;

    public CartV2ControllerTests()
    {
        _cartV2Controller = new CartController(_mockCartService.Object);
    ...
    public void GetCartInfo_ValidId_ReturnsOkResult()
    {
        ...
        var cart = new Cart { Id = cartId, CartItems = _fixture.CreateMany<CartItem>(3).ToList() };
        _mockCartService.Setup(s => s.GetCart(cartId)).Returns(cart);

        // Act
        var actualResult = _cartV2Controller.GetCartInfo(cartId);

        // Assert
        var okResult = Assert.IsType<OkObjectResult>(actualResult.Result);
        var returnValue = Assert.IsType<CartItemDto[]>(okResult.Value);
        Assert.Equal(cart.CartItems.Count, returnValue.Length);
    }
    ...
    public void AddItemToCart_ValidCartDto_ReturnsOkResult()
    {
        ...
        // Act
        var actualResult = _cartV2Controller.AddItemToCart(cartDto);
    ...
    public void DeleteItem_ExistingItem_ReturnsNoContent()
    {
        ...
        // Act
        var actualResult = _cartV2Controller.DeleteItem(cartId, itemId);
